...
CSS = """
  ...
  * { margin: 0; padding: 0; box-sizing: border-box; }
  ...
  /* ---------- Vyhledávání napříč vydáními ---------- */
  .hledani {
    background: var(--karta); border: 1px solid var(--okraj);
    border-radius: 16px; padding: 16px 18px; margin: 14px 0 6px;
    transition: background-color .45s ease, border-color .45s ease;
  }
  .hledani.skryta { display: none; }
  .hledani-pole {
    width: 100%; border: 1px solid var(--okraj); background: var(--pozadi);
    color: var(--text); border-radius: 999px; padding: 10px 18px;
    font-size: 0.95rem; outline: none;
    transition: border-color .2s ease, background-color .45s ease;
  }
  .hledani-pole:focus { border-color: var(--seda); }
  .hledani-vysledky { margin-top: 4px; }
  .hledani-radek {
    display: block; text-decoration: none; color: var(--text);
    padding: 10px 6px; border-bottom: 1px solid var(--linka);
    font-size: 0.92rem; line-height: 1.4; text-align: left;
  }
  .hledani-radek:last-child { border-bottom: 0; }
  .hledani-radek:hover { background: var(--pozadi); border-radius: 8px; }
  .hledani-radek mark {
    background: color-mix(in srgb, #b3541e 25%, transparent);
    color: inherit; border-radius: 3px; padding: 0 1px;
  }
  .hledani-meta { display: block; font-size: 0.75rem; color: var(--seda); margin-top: 2px; }
  .hledani-nic { padding: 12px 6px; font-size: 0.88rem; color: var(--seda); }

  /* ---------- Navigace mezi vydáními (archiv) ---------- */
  .vydani-nav { margin-top: 10px; font-size: 0.85rem; color: var(--seda); }
  .vydani-sipka { color: var(--seda); text-decoration: none; font-weight: 600; }
  .vydani-sipka:hover { color: var(--text); text-decoration: underline; }
  .archivni-stitek {
    font-weight: 700; text-transform: uppercase; letter-spacing: 0.05em;
    font-size: 0.75rem; color: #b3541e;
  }
  ...
  .akt p { font-size: 0.92rem; color: var(--odstavec); margin-top: 5px;
           transition: color .45s ease; }
  /* ---------- FT News Briefing ---------- */
  .ft-panel { border-left: 4px solid #f2dfce; }
  .ft-znak {
    font-family: Georgia, "Times New Roman", serif; font-weight: 700;
    background: #fff1e5; color: #33302e; border-radius: 6px;
    padding: 2px 8px; font-size: 1rem; letter-spacing: 0;
  }
  .ft-text { font-size: 0.95rem; color: var(--odstavec); margin: 10px 0 14px;
             transition: color .45s ease; }
  .ft-panel .akt { padding: 13px 0; }
  .ft-pata { padding-top: 14px; }

  .kauza-stitek {
    display: inline-block; vertical-align: middle; margin-left: 8px;
    font-size: 0.68rem; font-weight: 700; text-transform: uppercase;
    letter-spacing: 0.06em; color: #b3541e;
    background: color-mix(in srgb, #b3541e 12%, var(--karta));
    border: 1px solid color-mix(in srgb, #b3541e 25%, var(--karta));
    border-radius: 999px; padding: 2px 9px;
  }
  body.tmavy .kauza-stitek { color: #e0955c; }
  details.kauza { margin-top: 8px; }
  details.kauza summary {
    cursor: pointer; font-size: 0.8rem; font-weight: 700;
    text-transform: uppercase; letter-spacing: 0.06em; color: var(--seda);
    list-style: none; display: inline-flex; align-items: center; gap: 6px;
  }
  details.kauza summary::before { content: "▸"; transition: transform .2s ease; }
  details.kauza[open] summary::before { transform: rotate(90deg); }
  details.kauza summary:hover { color: var(--text); }
  details.kauza p {
    margin-top: 8px; font-size: 0.92rem; color: var(--odstavec);
    background: var(--pozadi); border-left: 3px solid var(--linka);
  ...
  .zdroj-dat { font-size: 0.7rem; margin-top: 8px; }
  .zdroj-dat a { color: var(--seda); text-decoration: none; }
  .zdroj-dat a:hover { text-decoration: underline; }
...
JS = """
  ...
  (function () {
    ...
    function prekresli() {
      // Jen položky v sekci Aktuality (FT blok má vlastní .akt řádky)
      var vTabu = Array.from(document.querySelectorAll(".aktuality .akt"))
      ...
      document.querySelectorAll(".aktuality .akt").forEach(a => a.classList.add("skryta"));
  ...
  // ---- Vyhledávání napříč vydáními ----
  (function () {
    var btn = document.getElementById("hledat-btn");
    var panel = document.getElementById("hledani");
    var pole = document.getElementById("hledani-pole");
    var vysledky = document.getElementById("hledani-vysledky");
    if (!btn) return;
    var index = null, nacitaSe = false;

    function normalizuj(s) {
      return s.toLowerCase().normalize("NFD").replace(/[\\u0300-\\u036f]/g, "");
    }

    function zvyrazni(text, dotaz) {
      var norm = normalizuj(text);
      var kde = norm.indexOf(dotaz);
      if (kde === -1) return document.createTextNode(text);
      var kus = document.createDocumentFragment();
      kus.appendChild(document.createTextNode(text.slice(0, kde)));
      var m = document.createElement("mark");
      m.textContent = text.slice(kde, kde + dotaz.length);
      kus.appendChild(m);
      kus.appendChild(document.createTextNode(text.slice(kde + dotaz.length)));
      return kus;
    }

    function hledej() {
      var dotaz = normalizuj(pole.value.trim());
      vysledky.innerHTML = "";
      if (dotaz.length < 2 || !index) return;
      var anglicky = document.body.classList.contains("anglicky");
      var shody = index.filter(z => z.n.indexOf(dotaz) !== -1);
      if (!shody.length) {
        var nic = document.createElement("div");
        nic.className = "hledani-nic";
        nic.textContent = anglicky ? "Nothing found." : "Nic nenalezeno.";
        vysledky.appendChild(nic);
        return;
      }
      shody.slice(0, 30).forEach(function (z) {
        var radek = document.createElement("a");
        radek.className = "hledani-radek";
        radek.href = ZAKLAD + z.u;
        radek.appendChild(zvyrazni(anglicky && z.te ? z.te : z.t, dotaz));
        var meta = document.createElement("span");
        meta.className = "hledani-meta";
        meta.textContent = z.d + " · " + (z.typ === "a"
          ? (anglicky ? "breaking" : "aktualita")
          : (anglicky ? "story" : "zpráva"));
        radek.appendChild(meta);
        vysledky.appendChild(radek);
      });
      if (shody.length > 30) {
        var vic = document.createElement("div");
        vic.className = "hledani-nic";
        vic.textContent = (anglicky ? "And " : "A dalších ")
          + (shody.length - 30)
          + (anglicky ? " more — refine your search." : " výsledků – upřesni hledání.");
        vysledky.appendChild(vic);
      }
    }

    btn.addEventListener("click", function () {
      panel.classList.toggle("skryta");
      if (!panel.classList.contains("skryta")) {
        pole.focus();
        if (!index && !nacitaSe) {
          nacitaSe = true;
          fetch(ZAKLAD + "hledani.json")
            .then(r => r.json())
            .then(function (data) { index = data; hledej(); })
            .catch(function () {
              vysledky.innerHTML = '<div class="hledani-nic">'
                + 'Vyhledávání funguje na webu (ne v lokálním souboru).</div>';
            });
        }
      }
    });
    pole.addEventListener("input", hledej);
    document.addEventListener("keydown", function (e) {
      if (e.key === "Escape" && !panel.classList.contains("skryta"))
        panel.classList.add("skryta");
    });
  })();
...
def _kostra(data, aktivni_stranka, obsah, zaklad="", navigace=None):
    """Společná kostra stránky: hlavička, přepínače, obsah, JS.

    zaklad   — prefix relativních odkazů ("" pro kořen, "../" pro archiv)
    navigace — {"starsi": url|None, "novejsi": url|None, "archivni": bool}
    """
    zpravy_akt = ' aktivni' if aktivni_stranka == 'zpravy' else ''
    data_akt = ' aktivni' if aktivni_stranka == 'data' else ''
    navigace = navigace or {}

    archivni_stitek = ""
    if navigace.get("archivni"):
        archivni_stitek = (' · <span class="archivni-stitek">'
                           '<span class="l-cs">archivní vydání</span>'
                           '<span class="l-en">archived edition</span></span>')
    sipky = []
    if navigace.get("starsi"):
        sipky.append(f'<a class="vydani-sipka" href="{navigace["starsi"]}">'
                     '‹ <span class="l-cs">starší vydání</span>'
                     '<span class="l-en">older edition</span></a>')
    if navigace.get("novejsi"):
        sipky.append(f'<a class="vydani-sipka" href="{navigace["novejsi"]}">'
                     '<span class="l-cs">novější vydání</span>'
                     '<span class="l-en">newer edition</span> ›</a>')
    radek_navigace = (f'\n    <div class="vydani-nav">{" · ".join(sipky)}</div>'
                      if sipky else "")
...
<meta name="description" content="Vše podstatné ze světa i Česka na jedno přečtení – zprávy, trhy a česká makrodata, každé ráno.">
<meta property="og:type" content="website">
<meta property="og:site_name" content="Denní briefing">
<meta property="og:title" content="Denní briefing – vše podstatné na jednom místě">
<meta property="og:description" content="Vše podstatné ze světa i Česka na jedno přečtení – zprávy, trhy a česká makrodata, každé ráno.">
<meta property="og:url" content="https://dennibriefing.cz/">
<meta property="og:image" content="https://dennibriefing.cz/og.png">
<meta property="og:image:width" content="1200">
<meta property="og:image:height" content="630">
<meta property="og:locale" content="cs_CZ">
<meta name="twitter:card" content="summary_large_image">
<link rel="icon" type="image/svg+xml" href="{zaklad}favicon.svg">
<style>{CSS}</style>
</head>
<body>
<div class="obal" id="obal">
  <div class="prepinace">
    <button class="prepinac" id="hledat-btn" title="Hledat ve vydáních / Search">🔍</button>
  ...
  <div class="hledani skryta" id="hledani">
    <input type="search" class="hledani-pole" id="hledani-pole"
           placeholder="Hledej ve všech vydáních… / Search all editions…">
    <div class="hledani-vysledky" id="hledani-vysledky"></div>
  </div>
  <header class="hlavicka">
    ...
    <div class="cas"><span class="l-cs">Vydáno</span><span class="l-en">Published</span> {data["vygenerovano"]}{archivni_stitek}</div>{radek_navigace}
  </header>
  <nav class="stranky">
    <div class="stranky-vnitrek">
      <a class="stranka{zpravy_akt}" href="{zaklad}index.html"><span class="l-cs">Zprávy</span><span class="l-en">News</span></a>
      <a class="stranka{data_akt}" href="{zaklad}data.html"><span class="l-cs">Data</span><span class="l-en">Data</span></a>
    </div>
  </nav>
{obsah}
</div>
<script>var ZAKLAD = "{zaklad}";{JS}</script>
</body>
</html>
"""
...
# ------------------------------------------------------------
def vytvor_html(data, zaklad="", navigace=None):
    zpravy = data["zpravy"]
    e = html_mod.escape
...
{chr(10).join(karty)}
    ...
    if aktuality:
        ...
        for a in aktuality:
            ...
            kauza = ""
            stitek_kauzy = ""
            if a.get("kauza_shrnuti"):
                stitek_kauzy = (' <span class="kauza-stitek">'
                                '<span class="l-cs">vývoj kauzy</span>'
                                '<span class="l-en">ongoing story</span></span>')
                kauza = f"""
        <details class="kauza">
          <summary><span class="l-cs">Shrnutí celé kauzy</span><span class="l-en">The story so far</span></summary>
          <p><span class="l-cs">{e(a['kauza_shrnuti'])}</span><span class="l-en">{e(a.get('kauza_shrnuti_en', a['kauza_shrnuti']))}</span></p>
        </details>"""
            polozky.append(f"""\
      <div class="akt{skryta}" data-oblast="{a['oblast']}">
        <h3><span class="l-cs">{e(a['titulek'])}</span><span class="l-en">{e(a.get('titulek_en', a['titulek']))}</span>{stitek_kauzy}</h3>
        <p><span class="l-cs">{e(a['shrnuti'])}</span><span class="l-en">{e(a.get('shrnuti_en', a['shrnuti']))}</span></p>{kauza}
...
{chr(10).join(polozky)}
    ...
    # ---- Blok FT News Briefing (dnešní epizoda podcastu) ----
    ft = data.get("ft_briefing")
    sekce_ft = ""
    if ft:
        # Nový formát: seznam témat; starý formát (jedno shrnutí) převedeme
        temata_ft = ft.get("temata") or (
            [{"nadpis": "", "shrnuti": ft["shrnuti"],
              "nadpis_en": "", "shrnuti_en": ft.get("shrnuti_en", ft["shrnuti"])}]
            if ft.get("shrnuti") else []
        )
        radky_ft = []
        for t in temata_ft:
            nadpis = ""
            if t.get("nadpis"):
                nadpis = (f'\n      <h3><span class="l-cs">{e(t["nadpis"])}</span>'
                          f'<span class="l-en">{e(t.get("nadpis_en", t["nadpis"]))}</span></h3>')
            radky_ft.append(f"""\
    <div class="akt">{nadpis}
      <p><span class="l-cs">{e(t['shrnuti'])}</span><span class="l-en">{e(t.get('shrnuti_en', t['shrnuti']))}</span></p>
    </div>""")
        sekce_ft = f"""
  <section class="panel ft-panel">
    <h2 class="sekce-titul"><span class="ft-znak">FT</span><span class="l-cs">News Briefing · o čem se dnes mluví</span><span class="l-en">News Briefing · today's episode</span></h2>
{chr(10).join(radky_ft)}
    <div class="ft-pata">
      <a class="zdroj-chip" href="{e(ft['odkaz'])}" target="_blank" rel="noopener">🎧 <span class="l-cs">Poslechnout epizodu</span><span class="l-en">Listen to the episode</span></a>
    </div>
  </section>"""

    obsah = f"""{sekce_aktualit}{sekce_ft}
  <nav class="filtry">
{chr(10).join("    " + t for t in tlacitka)}
  </nav>
{chr(10).join(sekce_rubrik)}"""
    return _kostra(data, "zpravy", obsah, zaklad=zaklad, navigace=navigace)
...
{chr(10).join(radky)}
    ...
    if trhy:
      ...
      <div class="rozsahy">
        {rozsahy}
            ...
            if t.get("zdroj_dat"):
                zdroj_dat = (f'\n      <div class="zdroj-dat">'
                             f'<a href="https://www.cnb.cz/arad/#/cs" '
                             f'target="_blank" rel="noopener">{e(t["zdroj_dat"])}'
                             f'</a></div>')
    ...
    <article class="ukazatel" data-i="{i}" data-nazev="{e(t['nazev'])}" data-vychozi="{vychozi}">
      ...
      <div class="ukazatel-radek"><span class="ukazatel-hodnota">{e(t['hodnota'])}</span>{zmena}</div>{graf}{zdroj_dat}
    </article>""")
...
def uloz_html(data):
    ...
    # Platná archivní vydání (nejnovější = právě vydané)
    archivni = []
    for soubor in sorted(glob_mod.glob("archiv/*.json")):
        try:
            with open(soubor, encoding="utf-8") as f:
                d = json.load(f)
            # jen kompletní vydání (ochrana proti poškozeným souborům)
            if (d.get("zpravy") and d.get("vygenerovano")
                    and all("tema" in z for z in d["zpravy"])):
                archivni.append((soubor, d))
        except (json.JSONDecodeError, OSError):
            continue

    # Hlavní stránka: šipka na předposlední vydání (poslední = aktuální)
    navigace_indexu = None
    if len(archivni) >= 2:
        predchozi = os.path.basename(archivni[-2][0]).replace(".json", ".html")
        navigace_indexu = {"starsi": f"archiv/{predchozi}"}

    with open("index.html", "w", encoding="utf-8") as f:
        f.write(vytvor_html(data, navigace=navigace_indexu))
    with open("data.html", "w", encoding="utf-8") as f:
        f.write(vytvor_data_html(data))

    # Stránky minulých vydání do složky archiv/
    for i, (soubor, d) in enumerate(archivni):
        starsi = None
        if i > 0:
            starsi = os.path.basename(archivni[i - 1][0]).replace(".json", ".html")
        if i < len(archivni) - 1:
            novejsi = os.path.basename(archivni[i + 1][0]).replace(".json", ".html")
        else:
            novejsi = "../index.html"
        html = vytvor_html(
            d, zaklad="../",
            navigace={"starsi": starsi, "novejsi": novejsi, "archivni": True},
        )
        cil = soubor.replace(".json", ".html")
        with open(cil, "w", encoding="utf-8") as f:
            f.write(html)

    # Index pro vyhledávání napříč všemi vydáními
    import unicodedata

    def _normalizuj(text):
        rozlozene = unicodedata.normalize("NFD", text.lower())
        return "".join(c for c in rozlozene if not unicodedata.combining(c))

    polozky_hledani = []
    zdroje_indexu = archivni if archivni else [("", data)]
    for i, (soubor, d) in enumerate(zdroje_indexu):
        if soubor and i < len(zdroje_indexu) - 1:
            url = "archiv/" + os.path.basename(soubor).replace(".json", ".html")
        else:
            url = "index.html"   # nejnovější vydání = hlavní stránka
        datum = " ".join(str(d.get("vygenerovano", "")).split()[:3])
        for z in d.get("zpravy", []):
            polozky_hledani.append({
                "t": z["titulek"], "te": z.get("titulek_en", ""),
                "d": datum, "u": url, "typ": "z",
                "n": _normalizuj(z["titulek"] + " "
                                 + z.get("titulek_en", "")),
            })
        for a in d.get("aktuality", []):
            polozky_hledani.append({
                "t": a["titulek"], "te": a.get("titulek_en", ""),
                "d": datum, "u": url, "typ": "a",
                "n": _normalizuj(a["titulek"] + " "
                                 + a.get("titulek_en", "")),
            })
    polozky_hledani.reverse()   # nejnovější výsledky první
    with open("hledani.json", "w", encoding="utf-8") as f:
        json.dump(polozky_hledani, f, ensure_ascii=False, separators=(",", ":"))

    print(f"Stránky uloženy: index.html + data.html"
          f" + {len(archivni)} archivních vydání"
          f" + hledání ({len(polozky_hledani)} položek).")

...
# Vše razítkujeme českým časem (GitHub servery běží v UTC)
try:
    from zoneinfo import ZoneInfo
    _PRAHA = ZoneInfo("Europe/Prague")
except Exception:
    _PRAHA = None


def _ted():
    return datetime.now(_PRAHA) if _PRAHA else datetime.now()
from stahni_zpravy import stahni_vsechny_zpravy

# Stropy délky odpovědí (včetně interního "přemýšlení" modelu, které se
# účtuje jako výstup) — chrání před nečekanou útratou
MAX_TOKENS_KARTY = 24000
MAX_TOKENS_AKTUALITY = 40000  # zvýšeno z 20000: briefing 12+12 událostí
                              # dvojjazyčně se na velkých dnech nevešel a padal
MAX_TOKENS_FAKTA = 16000

# Počítadlo spotřeby tokenů za běh
_SPOTREBA = {"vstup": 0, "vystup": 0}
...
def _nacti_cache():
    ...
    hranice = (_ted() - timedelta(days=CACHE_DNU)).strftime("%Y-%m-%d")
...
def _stahni_ft_epizodu():
    """Nejnovější epizoda podcastu FT News Briefing (max 48 h stará)."""
    try:
        import calendar
        import time as time_mod
        import feedparser
        from stahni_zpravy import _vycisti_text
        feed = feedparser.parse(FT_BRIEFING_FEED)
        if not feed.entries:
            duvod = getattr(feed, "bozo_exception", "feed je prázdný")
            print(f"  FT News Briefing: feed nevrátil žádné epizody ({duvod})")
            return None
        for polozka in feed.entries[:3]:
            cas = polozka.get("published_parsed")
            if cas and calendar.timegm(cas) < time_mod.time() - 48 * 3600:
                continue
            popis = _vycisti_text(polozka.get("summary", ""), max_delka=1500)
            if popis:
                print(f"  FT News Briefing: epizoda \"{polozka.get('title', '')[:60]}\"")
                return {"titulek": polozka.get("title", "").strip(),
                        "popis": popis,
                        "odkaz": polozka.get("link", "")}
        print("  FT News Briefing: žádná epizoda mladší 48 h (nebo bez popisu)")
    except Exception as chyba:
        print(f"  FT News Briefing: nedostupný ({chyba})")
    return None


def _nacti_predchozi_aktuality(max_vydani=3, max_radku=40):
    """Aktuality z minulých vydání (titulek + shrnutí) — podklad pro kauzy."""
    soubory = sorted(glob.glob(os.path.join("archiv", "*.json")))[-max_vydani:]
    radky, videne = [], set()
    for soubor in soubory:
        try:
            ...
            continue
        datum = d.get("vygenerovano", "")
        for a in d.get("aktuality", []):
            t = a.get("titulek", "")
            if t and t not in videne:
                videne.add(t)
                radky.append(f"[{datum}] {t} — {a.get('shrnuti', '')}")
    return radky[-max_radku:]
...
- V českých textech používej českou pomlčku „–", nikdy dlouhou „—"."""
...
def _surove_volani(client, prompt, max_tokens):
    ...
    try:
        _SPOTREBA["vstup"] += odpoved.usage.input_tokens
        _SPOTREBA["vystup"] += odpoved.usage.output_tokens
    except Exception:
        pass
...
def _zavolej_clauda(client, prompt, max_tokens=MAX_TOKENS_AKTUALITY):
    ...
    posledni_chyba = None
    for pokus in (1, 2):
        text = _surove_volani(client, prompt, max_tokens)
        try:
...
def vyber_a_preloz(zpravy):
    ...
    # Dnešní epizoda podcastu FT News Briefing (samostatný blok na webu)
    ft_epizoda = _stahni_ft_epizodu()
    if ft_epizoda:
        blok_ft = (
            "MEZIÚKOL — \"ft\": Níže je popis dnešní epizody podcastu FT News "
            "Briefing (Financial Times). Epizoda vždy pokrývá několik témat — "
            "rozděl je a vytvoř objekt {\"temata\": [...]}, kde každé téma je "
            "{\"nadpis\": krátký věcný český titulek (max 10 slov), "
            "\"shrnuti\": 1–2 věty česky, \"nadpis_en\": titulek anglicky, "
            "\"shrnuti_en\": 1–2 věty anglicky}. Obvykle 3–5 témat, v pořadí "
            "z popisu. Piš VÝHRADNĚ to, co je v popisu epizody — nic nedomýšlej.\n"
            f"EPIZODA: {ft_epizoda['titulek']} — {ft_epizoda['popis']}\n\n"
        )
        format_ft = ', "ft": {"temata": [...]}'
    else:
        blok_ft = ""
    ...
    # Aktuality z minulých vydání — podklad pro shrnutí kauz
    predchozi = _nacti_predchozi_aktuality()
    if predchozi:
        kauzy_uvod = ("níže v oddílu MINULÁ VYDÁNÍ máš přehled dřívějších "
                      "aktualit.")
        blok_videne += ("\n\nMINULÁ VYDÁNÍ (pro kauzy):\n"
                        + "\n".join("- " + r for r in predchozi))
    else:
        kauzy_uvod = ("(zatím není k dispozici archiv minulých vydání, "
                      "kauzy proto vytvářej jen z dnešních zpráv, pokud "
                      "z nich pokračování jasně vyplývá)")
...
{seznam_novych}"""
    ...
    if nove:
        ...
        surove_karty = _zavolej_clauda(client, prompt_karty, MAX_TOKENS_KARTY)
        dnes = _ted().strftime("%Y-%m-%d")
...
KAUZY (pokračující příběhy): {kauzy_uvod}
Pokud je dnešní událost pokračováním kauzy z minulých vydání, přidej k ní
navíc dvě pole:
   - "kauza_shrnuti": 2–4 věty shrnující CELOU kauzu od začátku do dneška
     (kontext z minulých vydání + dnešní vývoj) — čtenář, který kauzu
     nesledoval, musí po přečtení rozumět celému příběhu
   - "kauza_shrnuti_en": totéž přirozenou angličtinou
U zcela nových událostí tato pole VYNECH.

{blok_ft}DRUHÝ ÚKOL — "sledovat": krátký kalendář Co sledovat: 5–8 NADCHÁZEJÍCÍCH
...
{{"aktuality": [...], "sledovat": [...]{format_ft}}}, bez jakéhokoli dalšího textu.
Pozor na validitu JSON: uvozovky uvnitř textů escapuj jako \\" a nepoužívej
nezakončené řetězce.
...
{seznam}"""
    ...
    print(f"Volání 2/3: aktuality za 24 hodin + kalendář ({MODEL})...", flush=True)
    # Záchranná síť: kdyby aktuality z jakéhokoli důvodu selhaly (přetížení
    # API, příliš dlouhá odpověď, vadný JSON…), web se přesto vytvoří — jen
    # bez sekce Aktuality. Skutečná chyba se vypíše do logu k diagnostice.
    aktuality, sledovat, ft_briefing = [], [], None
    try:
        surova_odpoved = _zavolej_clauda(client, prompt_aktuality, MAX_TOKENS_AKTUALITY)
        ...
        surove_ft = surova_odpoved.get("ft")
        if ft_epizoda and isinstance(surove_ft, dict):
            temata_ft = [
                {"nadpis": t.get("nadpis", ""),
                 "shrnuti": t.get("shrnuti", ""),
                 "nadpis_en": t.get("nadpis_en", t.get("nadpis", "")),
                 "shrnuti_en": t.get("shrnuti_en", t.get("shrnuti", ""))}
                for t in surove_ft.get("temata", [])
                if t.get("nadpis") and t.get("shrnuti")
            ]
            if temata_ft:
                ft_briefing = {
                    "titulek": ft_epizoda["titulek"],
                    "odkaz": ft_epizoda["odkaz"],
                    "temata": temata_ft,
                }
        if ft_epizoda and not ft_briefing:
            print("  FT News Briefing: model nevrátil témata epizody (\"ft\").")
        sledovat = [
        ...
        for a in surove_aktuality:
            ...
            aktuality.append({
                ...
                "zdroje": zdroje,
                "kauza_shrnuti": a.get("kauza_shrnuti") or "",
                "kauza_shrnuti_en": a.get("kauza_shrnuti_en")
                                    or a.get("kauza_shrnuti") or "",
            })
        aktuality.sort(key=lambda a: a["dulezitost"], reverse=True)
    except (Exception, SystemExit) as chyba:
        import traceback as _tb
        print(f"  !! Volání 2/3 (aktuality) SELHALO: {chyba!r}", flush=True)
        print("  Web se přesto vytvoří, jen dnes bez sekce Aktuality.", flush=True)
        _tb.print_exc()
        aktuality, sledovat = [], []
    ...
    # Přehled spotřeby za celý běh (orientační cena dle sazeb Sonnet)
    vstup, vystup = _SPOTREBA["vstup"], _SPOTREBA["vystup"]
    cena = vstup / 1e6 * 3 + vystup / 1e6 * 15
    print(f"Spotřeba API: {vstup:,} tokenů vstup + {vystup:,} výstup "
          f"≈ ${cena:.2f}".replace(",", " "))

    return {"zpravy": vysledek, "aktuality": aktuality,
            "sledovat": sledovat, "ft_briefing": ft_briefing}
...
{chr(10).join(polozky)}"""
    ...
    kontrola = _zavolej_clauda(client, prompt, MAX_TOKENS_FAKTA)
...
def uloz_data(vysledek, soubor="data.json"):
    ...
    try:
        ...
        trhy = []
    try:
        from stahni_makro import stahni_makro_cisla, stahni_arad_cisla
        trhy.extend(stahni_makro_cisla())
        trhy.extend(stahni_arad_cisla())
    except Exception as chyba:
        print(f"  Makrodata ČSÚ/ČNB nedostupná ({chyba}).")
    ...
    data = {
        "vygenerovano": (lambda d: f"{d.day}. {d.month}. {d.year} {d:%H:%M}")(_ted()),
        "zpravy": vysledek["zpravy"],
        "aktuality": vysledek.get("aktuality", []),
        "sledovat": vysledek.get("sledovat", []),
        "ft_briefing": vysledek.get("ft_briefing"),
        "trhy": trhy,
    }
    ...
    archivni = _ted().strftime("archiv/%Y-%m-%d_%H%M.json")
